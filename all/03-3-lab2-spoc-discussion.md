...
>  
		bootloader首先初始化GDT,完成了段机制,在段机制下才能有效实现保护;
		然后需要对物理内存的探测和空闲物理内存的管理;
		最后,页表建立初始过程中需要建立各级页表并且设置CR0控寄存器中的31位(最高)来使能页
		CR0对应如下:		
...
> 
		make debug的makefile中加入-d int发现:
		check_exception old: 0xffffffff new 0xd
		check_exception old: 0xd new 0xd
		check_exception old: 0x8 new 0xd
		出现了double error,triple error,导致abort.
...
请写出一个translation程序（可基于python, ruby, C, C++，LISP等），输入是一个虚拟地址和一个物理地址，能够自动计算出对应的页目录项的index值,页目录项内容的值，页表项的index值，页表项内容的值。即(pde_idx, pde_ctx, pte_idx, pte_ctx)
...
参考的输出格式为：
```
		1100110110 0000101100 000001111100        336         (336-300 | 3)                
...
结果:
	...
	对应文件为pagetable2.cpp in33.txt 和 out33.txt
